...
module sync_fifo #(
  ...
  initial begin
    ...
    if (!(ALMOST_EMPTY_THRESHOLD < ALMOST_FULL_THRESHOLD)) begin
      $fatal(1, "Require ALMOST_EMPTY_THRESHOLD < ALMOST_FULL_THRESHOLD");
    end

    if (ALMOST_FULL_THRESHOLD > DEPTH) begin
      $fatal(1, "ALMOST_FULL_THRESHOLD must be <= DEPTH");
    end
  end
  ...
  function automatic logic [ADDR_W-1:0] ptr_inc(
